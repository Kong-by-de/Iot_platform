...
/setTempHigh 25.0 - Уведомлять если >25°C
/setTempLow 15.0 - Уведомлять если <15°C
/setHumHigh 60.0 - Уведомлять если влажность >60%
/setHumLow 30.0 - Уведомлять если влажность <30%
/showAlerts - Показать текущие настройки
/clearAlerts - Удалить все оповещения

🔗 *Управление устройствами:*
/addDevice sensor_01 - Добавить устройство
/myDevices - Мои устройства
/removeDevice sensor_01 - Удалить устройство

🎮 *Тестирование:*
/testHot - Тест высокой температуры
/testCold - Тест низкой температуры
/testHumid - Тест высокой влажности
/testDry - Тест низкой влажности

💡 *Совет:* Начните с добавления устройства командой /addDevice
)";
}
...
2. Устройство привязано? (/myDevices)
3. Оповещения настроены? (/showAlerts)
...
std::string Formatter::formatDeviceList(
  ...
  if (devices.empty()) {
    return "📭 *У вас нет привязанных устройств*\n\n"
           "Используйте /addDevice <id> чтобы добавить устройство";
  }
...
std::string Formatter::formatAlertSettings(const models::UserAlert& alert) {
  ...
  if (!hasSettings) {
    oss << "ℹ️ *Оповещения не настроены*\n\n"
        << "Используйте команды /setTempHigh, /setTempLow, "
        << "/setHumHigh, /setHumLow для настройки";
  }
